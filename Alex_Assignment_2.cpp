...
int PathCheck(int CurrentX, int CurrentY, int RCount, int CCount, char NPos, char EPos, char SPos, char WPos)
{
    int Direction; //Direction case indicator to be returned to main function
    int k=0; //Arbitrary variable for loop count

    if(CurrentX-1<0||CurrentX+1>=RCount||CurrentY-1<0||CurrentY+1>=CCount){ //If going out of bounds, perform a backtrack process.
        Direction=6;
        k++;
    }
    else if(NPos!='#' && NPos!='-' && NPos!='+' && k==0){ //Check clear path to North direction
        Direction=1;
        ...
        Direction=4;
        if(WPos=='G'){ //Check if Goal is to the West
            Direction=5;
        }
        k++;
    }
    else if(CurrentX-1<1||CurrentX+1>=RCount||CurrentY-1<0||CurrentY+1>=CCount){ //If going out of bounds, perform a backtrack process.
        Direction=6;
    }
...
int main()
{
    ...
    const int RCount=31; //Number of maze rows. Enter prior to running program
    const int CCount=46; //Number of maze columns. Enter prior to running program
    std::string MazeStrings[RCount]; //Temporary array of strings after maze.txt file read
    ...
    std::array<std::array<char,CCount>,RCount> MazeArray; //Char array defined for maze characters
    ...
    for(int i=0; i<RCount; i++){ //For loop to read maze.txt lines to array of strings
    ...
    for(int i=0; i<RCount; i++){ //For loop to convert array of strings to full array of char to manipulate for maze
    ...
    while(startcheck!=true){ //loop to check validity of user input start coordinates
        ...
        if(StartX<0||StartX>RCount||StartY<0||StartY>CCount){ //Check start coordinates within maze boundary
    ...
    while(goalcheck!=true){ //Loop to check validity of goal coordinates
        ...
        if(GoalX<0||GoalX>RCount||GoalY<0||GoalY>CCount){ //Check goal coordinates within maze boundary
    ...
    while(PathEnd!=true){ //While loop to run path search as long as PathEnd is not true
        ...
        int DirectCase = PathCheck(CurrentX, CurrentY, RCount, CCount, NPos, EPos, SPos, WPos);
        
        int BacktrackCheck=0;
    ...
    SolutionOut.open("/home/acf/Documents/ENPM809Y Files/Assignment 2/SolutionOutput.txt");
    
    if(PathEnd==true){
        for(int i=0; i<RCount; i++){ //For loop to test maze output
            for(int j=0; j<CCount; j++){
                SolutionOut<<MazeArray[i][j];
            }
            SolutionOut<<'\n';
        }
        ...
        SolutionOut<<"No solution exists between the Start and Goal coordinates";
        std::cout<<"No solution exists between the Start and Goal coordinates"<<std::endl;
    }
